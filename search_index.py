...
- Input Markdown files are generated by converter.py and include metadata headers
  ("# Title", "**Created:**", "**Updated:**", "**Conversation ID:**") plus
  message sections like "### **User**".
- SQLite must be built with FTS5 enabled for keyword search.
- Vector search requires embeddings built during conversion; sentence-transformers
  is optional (hashing fallback), and the embeddings file must exist to query.
- Index builds are typically enabled via CHATGPT_HISTORY_BUILD_SEARCH_INDEX=1 in
  converter.py.

Index artifacts:
- search.sqlite (FTS5 keyword index)
- search_meta.json (chunk metadata)
- search_embeddings.bin (optional vectors)
...
- CHATGPT_HISTORY_HASH_DIM=384
- CHATGPT_HISTORY_CHUNK_WORDS=320
- CHATGPT_HISTORY_CHUNK_OVERLAP=64
- CHATGPT_HISTORY_VECTOR_SCAN_LIMIT=20000
"""
