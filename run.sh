...
cleanup() {
  ...
  # Best-effort: terminate the whole job (python + any spawned browser processes).
  kill -- -$$ >/dev/null 2>&1
  exit 130
}
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    -h|--help)
      cat <<'EOF'
Usage: ./run.sh [--no-mermaid] [--stream] [--count-total] [--workers N] [--chrome-path /path/to/chrome]
